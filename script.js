document.addEventListener("DOMContentLoaded", () => {
  ...
  // y positions & rotations per card (keyframes)
  const transforms = [
  ...
  ScrollTrigger.create({
    ...
    onUpdate: (self) => {
      ...
      // FIX: use scrollWidth since element extends beyond viewport
      ...
      cards.forEach((card, index) => {
        ...
        // FIX: proper clamp (0..1)
        const cardProgress = gsap.utils.clamp(0, 1, (progress - delay) * 2);

        if (cardProgress > 0) {
          const cardStartX = 25;   // xPercent start (just off-screen right)
          const cardEndX = -650;   // xPercent end (fly left across)
          ...
          gsap.set(card, {
            ...
            opacity: 1, // FIX: correct prop
          });
        } else {
          gsap.set(card, { opacity: 0 }); // FIX: correct prop
        }
      });
    },
  });
